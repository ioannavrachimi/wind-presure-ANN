...
Matlab_data <- "C:/Users/xeb14148/Dropbox/Cp Paper/R code/Matlab file_wind_15"
...
#Start
wind_direction <- c(15) #Wind Direction
...
for(i in 1:length(Matlab_filenames)){
  for(j in 1:length(surfaces)) {
    ...
    assign(paste0(Matlab_filenames[i],"_surface_",j), data.frame(object[object$surface==j,c('x','y','cp','surface')])) #creation of dataframe for each model and surface
  }
}
...
for(j in 1:length(threshold_values)){
  for(i in 1:length(surfaces)) {
    ...
    trained_ANN <- readRDS(filename)
    pann_trained <- compute(trained_ANN, test_[,1:3])
